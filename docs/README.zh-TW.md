...
| 特性 | 說明 |
|------|------|
| 🧠 **跨會話記憶** | 踩過的坑、做過的決策、定下的規範，換個會話照樣記得，AI 終於不再失憶 |
| 🔍 **混合智能檢索** | FTS5 全文 + 向量語義雙路搜尋，RRF 融合排序 + 複合評分（時效×頻率×重要度），比純向量搜尋精準數倍 |
| 🐛 **問題追蹤** | 內建 Issue Tracker — 發現→排查→修復→歸檔完整閉環，AI 自動管理 bug 生命週期 |
| 📋 **任務管理** | Spec → 任務拆分 → 子任務巢狀 → 狀態同步 → 聯動歸檔，AI 驅動完整開發流程 |
| 🚦 **會話狀態** | 阻塞管理 + 斷點續接 + 進度追蹤，換會話 / context 壓縮後無縫銜接 |
| 🪝 **Hooks + Steering** | 自動注入工作規則 + 行為守衛鉤子，AI 行為一致性有保障，不用每次重複交代 |
| 🧬 **記憶進化** | 矛盾檢測自動替代舊知識 + 短期→長期自動晉升 + 90天未用自動歸檔，記憶庫自我進化 |
| 📊 **桌面端 + Web 看板** | 原生桌面應用（macOS/Windows/Linux）+ Web 看板，3D 向量網路一眼看清知識關聯 |
| 💰 **省 50%+ Token** | 語義檢索按需召回，告別每次對話複製貼上專案背景的全量上下文注入 |
| 🏠 **完全本地** | 零依賴雲端服務，ONNX 本地推理，無需 API Key，資料不出你的電腦 |
| 🔌 **11 IDE 全覆蓋** | Cursor / Kiro / Claude Code / Windsurf / VSCode / Copilot / OpenCode / Trae / Codex / Antigravity / OpenClaw — 一鍵安裝卸載 |
| 📁 **多專案隔離** | 一個 DB 管所有專案，自動隔離互不干擾，切換專案無感知 |
| 🔄 **智慧去重** | 相似度 > 0.95 自動合併更新，記憶庫永遠乾淨，不會越用越亂 |
| 🌐 **7 語言支援** | 簡體中文 / 繁體中文 / English / Español / Deutsch / Français / 日本語，看板 + Steering 全棧國際化 |
...
**補丁：對抗 Opus 4.7 預設傾向的規則硬約束**

- 🚫 §1 新增 **禁止澄清式反問**：祈使句禁止反問「分階段還是一次推進 / 全量還是部分 / 要不要 X / 先做 A 還是先做 B」，含歧義按最完整範圍執行
- 🚫 §1 新增 **禁止防禦性彙報**：禁止用「按指令保留」「標 pending」「非關鍵路徑」「未必要的子測」「後續迭代」等措辭為未執行項開脫
- 📋 §1 新增 **彙報格式**：禁止 Phase A/B/C/D 清單 + 「最終狀態」 + 「未做（按指令保留）」 三段式，使用者指令「全量做完」時禁列「未做/保留」章節
- 🎯 根因：對抗 Opus 4.7 模型相比 4.6 的預設「防禦性彙報」「澄清式反問」「結構化清單」傾向
- 🔄 7 語言規則文件（STEERING_CONTENT + DEV_WORKFLOW_PROMPT）全量同步 CLAUDE.md v2.4.5 更新

### v2.4.4

**補丁：訊息處理流程完整 A-I 對齊**

- 🧩 CLAUDE.md §4 訊息處理流程 B 部分路由展開：所有 4 條分支（閒聊/糾錯/偏好/其他）統一到 I(使用者確認歸檔) 終點，消除「僅寫到 F 就停」的流程不完整
- ⚙️ inject-workflow-rules.sh 訊息類型判斷章節與 §4 B 完全對齊：4 條路由粒度一致
- 🔧 修復 3 處衝突：路由粒度不一致（原 2 條 vs 4 條）/ B 與 E 職責混淆（「方案+阻塞」錯位）/ G/H/I 流程缺失
- 📝 違規條款統一措辭：「未輸出判斷結果就進入 C/D/E/F 步驟 = 違規」
- 🔄 7 語言規則文件（STEERING_CONTENT + DEV_WORKFLOW_PROMPT）全量同步 CLAUDE.md v2.4.4 更新

### v2.4.3

**補丁：規則執行強化 & 圖譜視覺化**
- 🧠 §4.B：兩步必須結構（理解訊息 → 判斷類型），跳過即違規
- 📋 §8：審查 + 阻塞嵌入每個 Spec 步驟，跳過即違規
- 🧬 DEV_WORKFLOW_PROMPT：新增排查和程式碼修改階段的 graph trace/batch/add_node 規則
- 📊 圖譜看板：動態力佈局縮放、邊標籤預設隱藏（懸停顯示）、節點標籤碰撞偵測
- 🔧 移除 DEV_WORKFLOW_PROMPT 中冗餘的「高頻違規提醒」章節
- 📝 統一 7 種語言的 §1 身份與語氣（不翻譯）
