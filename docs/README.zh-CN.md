...
| 🔍 **混合智能检索** | FTS5 全文 + 向量语义双路搜索，RRF 融合排序 + 复合评分（时效×频率×重要度），比纯向量搜索精准数倍 |
| 🐛 **问题追踪** | 内置 Issue Tracker — 发现→排查→修复→归档完整闭环，AI 自动管理 bug 生命周期 |
| 📋 **任务管理** | Spec → 任务拆分 → 子任务嵌套 → 状态同步 → 联动归档，AI 驱动完整开发流程 |
| 🚦 **会话状态** | 阻塞管理 + 断点续接 + 进度追踪，换会话 / context 压缩后无缝衔接 |
| 🪝 **Hooks + Steering** | 自动注入工作规则 + 行为守卫钩子，AI 行为一致性有保障，不用每次重复交代 |
| 🧬 **记忆进化** | 矛盾检测自动替代旧知识 + 短期→长期自动晋升 + 90天未用自动归档，记忆库自我进化 |
| 📊 **桌面端 + Web 看板** | 原生桌面应用（macOS/Windows/Linux）+ Web 看板，3D 向量网络一眼看清知识关联 |
| 💰 **省 50%+ Token** | 语义检索按需召回，告别每次对话复制粘贴项目背景的全量上下文注入 |
| 🏠 **完全本地** | 零依赖云服务，ONNX 本地推理，无需 API Key，数据不出你的电脑 |
| 🔌 **11 IDE 全覆盖** | Cursor / Kiro / Claude Code / Windsurf / VSCode / Copilot / OpenCode / Trae / Codex / Antigravity / OpenClaw — 一键安装卸载 |
...
**补丁：对抗 Opus 4.7 默认倾向的规则硬约束**

- 🚫 §1 新增 **禁止澄清式反问**：祈使句禁止反问"分阶段还是一次推进 / 全量还是部分 / 要不要 X / 先做 A 还是先做 B"，含歧义按最完整范围执行
- 🚫 §1 新增 **禁止防御性汇报**：禁止用"按指令保留""标 pending""非关键路径""未必要的子测""后续迭代"等措辞为未执行项开脱
- 📋 §1 新增 **汇报格式**：禁止 Phase A/B/C/D 清单 + "最终状态" + "未做（按指令保留）" 三段式，用户指令"全量做完"时禁列"未做/保留"章节
- 🎯 根因：对抗 Opus 4.7 模型相比 4.6 的默认"防御性汇报""澄清式反问""结构化清单"倾向
- 🔄 7 语言规则文件（STEERING_CONTENT + DEV_WORKFLOW_PROMPT）全量同步 CLAUDE.md v2.4.5 更新

### v2.4.4

**补丁：消息处理流程完整 A-I 对齐**

- 🧩 CLAUDE.md §4 消息处理流程 B 部分路由展开：所有 4 条分支（闲聊/纠错/偏好/其他）统一到 I(用户确认归档) 终点，消除"仅写到 F 就停"的流程不完整
- ⚙️ inject-workflow-rules.sh 消息类型判断章节与 §4 B 完全对齐：4 条路由粒度一致
- 🔧 修复 3 处冲突：路由粒度不一致（原 2 条 vs 4 条）/ B 与 E 职责混淆（"方案+阻塞"错位）/ G/H/I 流程缺失
- 📝 违规条款统一措辞："未输出判断结果就进入 C/D/E/F 步骤 = 违规"
- 🔄 7 语言规则文件（STEERING_CONTENT + DEV_WORKFLOW_PROMPT）全量同步 CLAUDE.md v2.4.4 更新

### v2.4.3

**补丁：规则执行强化 & 图谱可视化**
- 🧠 §4.B：两步必须结构（理解消息 → 判断类型），跳过即违规
- 📋 §8：审查 + 阻塞嵌入每个 Spec 步骤，跳过即违规
- 🧬 DEV_WORKFLOW_PROMPT：新增排查和代码修改阶段的 graph trace/batch/add_node 规则
- 📊 图谱看板：动态力布局缩放、边标签默认隐藏（悬停显示）、节点标签碰撞检测
- 🔧 移除 DEV_WORKFLOW_PROMPT 中冗余的"高频违规提醒"章节
- 📝 统一 7 种语言的 §1 身份与语气（不翻译）

### v2.4.1

**补丁：i18n 规则同步**
- 🔄 7 种语言规则文件（STEERING_CONTENT + DEV_WORKFLOW_PROMPT）全部同步 CLAUDE.md v2.4.0 更新
- 🧬 所有 steering 规则中添加 `graph` 工具引用（trace/batch/add_node/add_edge/remove）
- ✏️ 所有语言角色从"首席架构师"更新为"项目负责人"
- 📝 所有语言新增 3 条高频违规项 + 2 条禁止项
