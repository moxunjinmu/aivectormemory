...
**⚠️ 使用者提到「不對/不行/沒有/報錯/有問題」等否定詞 → 預設繼續 C 記錄問題，禁止自行判斷「設計如此」或「不是 bug」而跳過記錄。即使排查後確認非 bug，也必須先記錄再在排查中說明。**

**C. `track create`** — 發現即記錄（禁止先修再補）
...
- **回流檢查**：若在 task 執行中發現的 bug，歸檔後回到第8節繼續，`task update` 更新當前任務狀態並同步 tasks.md
...
歸檔後必須能看到完整記錄：
- `create`：content（現象+背景），禁止只傳 title 不傳 content
- 排查後 `update`：investigation（排查過程）、root_cause（根因）
