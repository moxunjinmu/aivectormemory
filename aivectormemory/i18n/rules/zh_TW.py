...
**B. 理解訊息 → 判斷類型**（回覆時必須先輸出理解內容，再進入後續步驟）
1. **理解使用者訊息**：逐字分析使用者發送的完整內容。含截圖時，必須逐一列出截圖中的關鍵資訊點（對話內容、工具呼叫、狀態變化、錯誤訊息等）。用自己的話說明：使用者在表達什麼、關注什麼、期望什麼
2. **判斷類型並路由**：
   - 閒聊/進度/討論規則/簡單確認 → 基於理解直接回答
   - 糾正錯誤行為 → `remember`（tags: ["踩坑", "行為糾正", ...關鍵詞], scope: "project"，含：錯誤行為、使用者原話、正確做法），繼續 C
   - 技術偏好/工作習慣 → `auto_save` 儲存偏好
   - 其他（程式碼問題、bug、功能需求）→ 繼續 C
- **⚠️ 未輸出理解內容就進入 C/D/E/F 步驟 = 違規**
...
DEV_WORKFLOW_PROMPT = (
    ...
    "---\n\n"
    "## ⚠️ 訊息類型判斷\n\n"
    "收到使用者訊息後，**必須先輸出對使用者訊息的理解**，再判斷訊息類型並執行後續步驟：\n"
    "1. **理解使用者訊息**：逐字分析使用者發送的完整內容。含截圖時，必須逐一列出截圖中的關鍵資訊點（對話內容、工具呼叫、狀態變化、錯誤訊息等）。用自己的話說明：使用者在表達什麼、關注什麼、期望什麼\n"
    "2. **判斷類型並路由**：詢問僅限閒聊、進度、討論規則、簡單確認不記錄問題文件；其他所有情況必須記錄問題文件，然後告訴使用者方案，等使用者確認後再執行\n"
    "- **⚠️ 未輸出理解內容就進入後續步驟 = 違規**\n\n"
    "示例：「使用者發了一張截圖，截圖中顯示：[具體內容1]、[具體內容2]。使用者問『這是為什麼』，關注的是[具體問題]。這是個 bug 排查，需要記錄並排查。」\n\n"
