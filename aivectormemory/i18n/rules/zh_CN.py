...
**B. 理解消息 → 判断类型**（回复时必须先输出理解内容，再进入后续步骤）
1. **理解用户消息**：逐字分析用户发送的完整内容。含截图时，必须逐一列出截图中的关键信息点（对话内容、工具调用、状态变化、错误信息等）。用自己的话说明：用户在表达什么、关注什么、期望什么
2. **判断类型并路由**：
   - 闲聊/进度/讨论规则/简单确认 → 基于理解直接回答
   - 纠正错误行为 → `remember`（tags: ["踩坑", "行为纠正", ...关键词], scope: "project"，含：错误行为、用户原话、正确做法），继续 C
   - 技术偏好/工作习惯 → `auto_save` 存储偏好
   - 其他（代码问题、bug、功能需求）→ 继续 C
- **⚠️ 未输出理解内容就进入 C/D/E/F 步骤 = 违规**
...
DEV_WORKFLOW_PROMPT = (
    ...
    "---\n\n"
    "## ⚠️ 消息类型判断\n\n"
    "收到用户消息后，**必须先输出对用户消息的理解**，再判断消息类型并执行后续步骤：\n"
    "1. **理解用户消息**：逐字分析用户发送的完整内容。含截图时，必须逐一列出截图中的关键信息点（对话内容、工具调用、状态变化、错误信息等）。用自己的话说明：用户在表达什么、关注什么、期望什么\n"
    "2. **判断类型并路由**：询问仅限闲聊、进度、讨论规则、简单确认不记录问题文档；其他所有情况必须记录问题文档，然后告诉用户方案，等用户确认后再执行\n"
    "- **⚠️ 未输出理解内容就进入后续步骤 = 违规**\n\n"
    "示例：\"用户发了一张截图，截图中显示：[具体内容1]、[具体内容2]。用户问'这是为什么'，关注的是[具体问题]。这是个 bug 排查，需要记录并排查。\"\n\n"
