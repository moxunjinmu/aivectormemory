...
**⚠️ 消息处理必须严格按 5→13 顺序执行，禁止跳步、省略、合并步骤。每个步骤完成后才能进入下一步。**
**⚠️ 用户提到"不对/不行/没有/报错/有问题"等否定词 → 默认执行第 7 步记录问题，禁止自行判断"设计如此"或"不是 bug"而跳过记录。即使排查后确认非 bug，也必须先记录再在排查中说明。**
...
- 纠正错误行为 → 更新 steering `<!-- custom-rules -->` 块（记录：错误行为、用户原话、正确做法），继续第 7 步
- 技术偏好/工作习惯 → `auto_save` 存储偏好
- 其他（代码问题、bug、功能需求）→ 继续第 7 步

示例："这是个询问，验证相应文件代码后回答"、"这是个问题，方案如下..."、"这个问题需要记录"

---

## ⚠️ 7. track create — 发现即记录

禁止先修再补。
- `content` 必填现象和背景，禁止只传 title 留空 content
- `status` 更新 pending
- 禁止字段留空
- 一次只修一个问题。新问题：不阻塞当前→记录继续；阻塞当前→先处理
...
⛔ 以下 11.1-11.4 必须全部完成才能进入第 12 步，任何一项未完成禁止设阻塞或汇报结果。**修改代码后立即执行，不要等用户提醒。**

**11.1 运行测试（立即执行，禁止跳过）** — 根据改动影响范围选择测试方式：
  ...
  前端测试：**仅 Playwright MCP**（browser_navigate → 交互 → browser_snapshot），其他一切方式（curl、脚本、node -e、截图、`open` 命令）均为违规。测试后不调用 browser_close。**Playwright MCP 工具在 deferred tools 列表中，使用前用 ToolSearch 加载。禁止假设工具不可用，禁止用 `open` 命令或让用户手动打开浏览器替代。**

**11.2 检查副作用（立即执行）** — grep 改动涉及的函数/变量名，确认其他调用方不受影响

**11.3 新问题处理** — 测试中发现非预期行为：阻塞当前 → 立即修复再继续；不阻塞 → `track create` 记录后继续

**11.4 track update（立即执行）** — 填 solution（方案）+ files_changed（JSON 数组）+ test_result（测试结果）。禁止字段留空。
...
仅 11.1-11.4 全部完成后才能 `status` 设阻塞（block_reason: "修复完成等待验证"或"需要用户决策"）
...
- 归档后必须能看到完整记录（content + investigation + root_cause + solution + files_changed + test_result）
- **回流检查**：若在 task 执行中发现的 bug，归档后回到第 16 节继续，`task update` 更新当前任务状态并同步 tasks.md
...
DEV_WORKFLOW_PROMPT = (
    ...
    "- **正确做法**：SQL 写入 `.sql` 文件用 `< data/xxx.sql` 执行；Python 验证脚本写成 .py 文件用 `python3 xxx.py` 执行；端口检查用 `lsof -ti:端口` + ignoreWarning:true\n\n"
    "---\n\n"
    "## ⚠️ 11. 代码修改后强制检查（每次修改代码后必须逐条执行）\n\n"
    "修改代码文件后，必须按顺序完成以下检查，**任何一项未完成都不能设阻塞或汇报结果**：\n\n"
    "1. **运行测试（立即执行，禁止跳过）** — 后端 pytest/curl，前端仅 Playwright MCP（navigate→交互→snapshot，不 close；工具在 deferred tools 列表中，用 ToolSearch 加载，禁止假设不可用）。跳过 = 违规。**禁止说\"请验证\"让用户操作，必须自己执行测试。**\n"
    "2. **检查副作用（立即执行）** — grep 改动涉及的函数/变量名，确认其他调用方不受影响\n"
    "3. **新问题处理** — 测试中发现非预期行为：阻塞当前→立即修复再继续；不阻塞→`track create` 记录后继续\n"
    "4. **track update（立即执行）** — 填 solution + files_changed + test_result\n"
    "5. 以上全部完成后，才能 `status` 设阻塞\"等待验证\"\n\n"
    "仅修改文档/配置文件（.md/.json/.yaml/.toml/.sh 等非代码文件）时不要求执行此清单。\n\n"
    "---\n\n"
    "## ⚠️ 违规示例（以下行为严格禁止）\n\n"
    "- ❌ 修改代码后直接说\"等待验证\" → 必须先完成第 11 条检查清单\n"
