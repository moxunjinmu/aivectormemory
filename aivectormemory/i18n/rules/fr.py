...
## ⚠️ 4. Flux de Traitement des Messages (Général)

**⚠️ Le traitement des messages doit suivre strictement les étapes 5→13 dans l'ordre, pas de saut, d'omission ou de fusion d'étapes. Chaque étape doit être terminée avant de passer à la suivante.**
**⚠️ Lorsque l'utilisateur mentionne des mots négatifs comme « incorrect/ne marche pas/manquant/erreur/a un problème » → par défaut exécuter l'étape 7 pour enregistrer le problème, interdit de juger soi-même « c'est par conception » ou « ce n'est pas un bug » et sauter l'enregistrement. Même si l'investigation confirme que ce n'est pas un bug, il faut d'abord enregistrer puis expliquer dans l'investigation.**

---

## ⚠️ 5. status vérifier le blocage

Bloqué → signaler et attendre, aucune action autorisée

---

## ⚠️ 6. Déterminer le type de message

Indiquer le résultat du jugement en langage naturel dans la réponse :
- Discussion informelle / progrès / discussion de règles / confirmation simple → répondre directement, flux terminé
- Correction de mauvais comportement → mettre à jour le bloc steering `<!-- custom-rules -->` (enregistrer : mauvais comportement, paroles de l'utilisateur, approche correcte), continuer à l'étape 7
- Préférences techniques / habitudes de travail → `auto_save` pour stocker les préférences
- Autre (problèmes de code, bugs, demandes de fonctionnalités) → continuer à l'étape 7
...
## ⚠️ 7. track create — enregistrer dès découverte

Interdit de corriger avant d'enregistrer.
...
## ⚠️ 9. Présenter la solution, établir blocage et attendre confirmation

- Correction simple → étape 10, multi-étapes → Section 16 (Spec)
...
## ⚠️ 10. Modifier le code après confirmation utilisateur
...
## ⚠️ 11. Vérifications obligatoires après modification du code

⛔ Les étapes 11.1-11.4 doivent TOUTES être complétées avant de passer à l'étape 12. Établir blocage ou rapporter résultats avec étape incomplète = violation. **Exécuter immédiatement après modification du code, ne pas attendre le rappel de l'utilisateur.**

**11.1 Exécuter les tests (exécuter immédiatement, interdit de sauter)** — choisir la méthode de test selon la portée de l'impact :
  ...
  Sauter = violation. **Interdit de dire « veuillez vérifier » à l'utilisateur — vous devez exécuter les tests vous-même.**
  Tests frontend : **uniquement Playwright MCP** (browser_navigate → interaction → browser_snapshot), toute autre méthode (curl, scripts, node -e, captures d'écran, commande `open`) est une violation. Ne pas appeler browser_close après les tests. **Les outils Playwright MCP sont dans la liste des deferred tools — utilisez ToolSearch pour les charger avant utilisation. Ne supposez jamais que les outils ne sont pas disponibles. N'utilisez jamais la commande `open` et ne demandez jamais à l'utilisateur d'ouvrir un navigateur manuellement.**

**11.2 Vérifier les effets secondaires (exécuter immédiatement)** — grep les noms de fonctions/variables modifiés, confirmer que les autres appelants ne sont pas affectés

**11.3 Gérer les nouveaux problèmes** — comportement inattendu pendant les tests : bloque l'actuel → corriger immédiatement et continuer ; ne bloque pas → `track create` pour enregistrer et continuer

**11.4 track update (exécuter immédiatement)** — remplir solution (solution) + files_changed (tableau JSON) + test_result (résultats de test). Interdit de laisser les champs vides.

⛔ Seuls les fichiers de documentation/configuration (.md/.json/.yaml/.toml/.sh etc.) sont exemptés de cette checklist.

---

## ⚠️ 12. Établir blocage pour vérification

Uniquement après que TOUTES les étapes 11.1-11.4 sont complétées, `status` peut établir le blocage (block_reason: "Correction terminée, en attente de vérification" ou "Décision utilisateur nécessaire")

---

## ⚠️ 13. Confirmation utilisateur et archivage

- `track archive` archiver, `status` effacer le blocage (is_blocked: false)
- L'archive doit montrer un enregistrement complet (content + investigation + root_cause + solution + files_changed + test_result)
- **Vérification de reflux** : si bug trouvé pendant l'exécution de task, après archivage retourner à la Section 16 pour continuer, `task update` mettre à jour l'état de la tâche courante et synchroniser tasks.md
- Avant fin de session → `auto_save` extraire automatiquement les préférences

---

## ⚠️ 14. Règles de Blocage
...
## ⚠️ 15. Vérifications Pré-opération
...
## ⚠️ 16. Spec et Gestion des Tâches (task)
...
## ⚠️ 17. Exigences de Qualité de Mémoire
...
## ⚠️ 18. Référence Rapide des Outils
...
## ⚠️ 19. Standards de Développement
...
DEV_WORKFLOW_PROMPT = (
    ...
    "- **Interdit** : traduire les messages de l'utilisateur, répéter ce que l'utilisateur a déjà dit, résumer les discussions dans une autre langue\n\n"
    "---\n\n"
    "## ⚠️ 6. Jugement du Type de Message\n\n"
    ...
    "- **Approche correcte** : écrire SQL dans un fichier `.sql` et utiliser `< data/xxx.sql` ; écrire les scripts de vérification Python comme fichiers .py et exécuter avec `python3 xxx.py` ; utiliser `lsof -ti:port` + ignoreWarning:true pour les vérifications de port\n\n"
    "---\n\n"
    "## ⚠️ 11. Checklist obligatoire après modification du code (exécuter après CHAQUE modification de code)\n\n"
    ...
    "1. **Exécuter les tests (exécuter immédiatement, interdit de sauter)** — backend : pytest/curl, frontend : UNIQUEMENT Playwright MCP (navigate→interaction→snapshot, pas de close ; outils dans la liste deferred tools, charger avec ToolSearch, ne jamais supposer indisponible). Sauter = violation. **Interdit de dire « veuillez vérifier » à l'utilisateur — vous devez exécuter les tests vous-même.**\n"
    "2. **Vérifier les effets secondaires (exécuter immédiatement)** — grep les noms de fonctions/variables modifiés, confirmer que les autres appelants ne sont pas affectés\n"
    ...
    "4. **track update (exécuter immédiatement)** — remplir solution + files_changed + test_result\n"
    ...
    "- ❌ Dire \"en attente de vérification\" sans compléter les tests → doit compléter la checklist de l'étape 11 d'abord\n"
